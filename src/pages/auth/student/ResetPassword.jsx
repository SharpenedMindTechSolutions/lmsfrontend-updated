...
export default function ResetPassword() {
  ...
  const submit = async e => {
    ...
    setLoading(true);
    try {
      await api.post('/reset-password/student', { token, email, newPassword: form.newPassword });
      setDone(true);
        ...
        <div className="card" style={{ padding:'32px 30px' }}>
          {done ? (
            <div style={{ textAlign:'center' }}>
              ...
              <button onClick={() => nav('/login')} className="btn btn-primary btn-full" style={{ marginTop:20 }}>
                Go to Login
              </button>
            </div>
          ) : (
